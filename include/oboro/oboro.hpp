...
inline void     print(lua_State* L, int i) noexcept {
  ...
  switch (t) {
    ...
    case LUA_TNIL:
      break;
    case LUA_TBOOLEAN:       ::std::printf("%d", lua_toboolean(L, i));
      break;
    case LUA_TLIGHTUSERDATA: ::std::printf("%p", lua_touserdata(L, i));
      break;
    case LUA_TNUMBER:        ::std::printf("%+lf", lua_tonumber(L, i));
      break;
    case LUA_TSTRING:        ::std::printf("[[%s]]", lua_tostring(L, i));
      break;
    case LUA_TTABLE:         ::std::printf("%p", lua_topointer(L, i));
      break;
    case LUA_TFUNCTION:
      ...
      break;
    case LUA_TUSERDATA:      ::std::printf("%p", lua_touserdata(L, i));
      break;
    case LUA_TTHREAD:        ::std::printf("%p", lua_tothread(L, i));
      break;
    default:
      break;
  }
...
inline T         gettable(lua_State* L, int a_TblIdx, const char* a_Key) {
  ...
  try {
    ...
    ::std::fprintf(stderr,
                   "ERROR!: oboro::gettable<%s, %s>(%p, %d, \"%s\");\n",
                   typeid(T).name(), (POP ? "true" : "false"),
                   L, a_TblIdx, a_Key);
    throw;
  }
